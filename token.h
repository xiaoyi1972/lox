...
namespace Lox {
	...
	namespace details {
		...
		template <class T, class R, class unpacked, class... left> struct unpacker {};

		template <class T, class R, class... unpacked, class left0, class... left>
		struct unpacker<T, R, std::tuple<unpacked...>, std::tuple<left0, left...>> {
			R operator()(T* vm, std::size_t index,
				...
				return next_unpacker()(
					...
					std::tuple_cat(std::move(t),
						std::tuple<left0>{std::get<left0>(
							vm->stack[vm->stack.top - index +
							std::size_t(sizeof...(unpacked))])}));
			}
		};

		template <class T, class R, class... unpacked>
		struct unpacker<T, R, std::tuple<unpacked...>, std::tuple<>> {
			R operator()(T* vm, std::size_t index, const std::function<R(unpacked...)>& f,
				std::tuple<unpacked...> t) const {
