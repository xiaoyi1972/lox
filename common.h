    ...
    struct VM {
        ...
        template <bool infix = true> void selfInc(TokenType operatorType) {
            if (previousOp) {
                ...
                if (isVariable) {
                    emitBytes(OpCode::INC, infix << 1 | operatorType == TokenType::dec);
                }
                else
                    error("Invalid assignment target.");
            }
            ...
            if (std::holds_alternative<Object>(callee)) {
                ...
                if (std::holds_alternative<std::shared_ptr<Closure>>(obj)) {
                    ...
                    if (argCount != nativeFunc->arity) {
                        runtimeError("Expected %d arguments but got %d.", nativeFunc->arity,
                            argCount);
                        return false;
                    }
                ...
                else if (std::holds_alternative<std::shared_ptr<Class>>(obj)) {
                    auto klass = std::get<std::shared_ptr<Class>>(obj);
                    ...
                    if (auto it = klass->methods.find(VM::initString);
                        ...
                        auto& initializer = std::get<std::shared_ptr<Closure>>(
                            std::get<Object>((it->second)));
                        return call(initializer.get(), argCount);
                    }
                    return true;
                }
                else if (std::holds_alternative<std::shared_ptr<BoundMethod>>(obj)) {
                    auto bound = std::get<std::shared_ptr<BoundMethod>>(obj);
        ...
        void method() {
            ...
            if (parser.previous.lexem == VM::initString) {
        ...
        void defineMethod(const std::string& name) {
            auto method = peek(0);
            auto klass = std::get<std::shared_ptr<Class>>(std::get<Object>(peek(1)));
        ...
        bool bindMethod(Class* klass, const std::string_view& name) {
            ...
            auto bound = std::make_shared<BoundMethod>(
                peek(0), std::get<std::shared_ptr<Closure>>(std::get<Object>(method)));
            pop();
            push(bound);
            return true;
        }
        ...
        bool invoke(const std::string& name, int argCount) {
            ...
            if (!std::holds_alternative<Object>(receiver) ||
                !std::holds_alternative<std::shared_ptr<Instance>>(
                    std::get<Object>(receiver))) {
                runtimeError("Only instances have methods.");
                return false;
            }

            auto instance =
                std::get<std::shared_ptr<Instance>>(std::get<Object>(receiver)).get();
        ...
        bool invokeFromClass(Class* klass, const std::string& name, int argCount) {
            if (auto it = klass->methods.find(std::string_view{ name });
                it != klass->methods.end()) {
                auto method =
                    std::get<std::shared_ptr<Closure>>(std::get<Object>(it->second))
                    .get();
    ...
    push(a op b); \
            ...
            for (;;) {
                ...
                if (std::holds_alternative<OpCode>(instruction))
                    ...
                    case OpCode::CLOSURE: {
                        auto function = std::get<std::shared_ptr<Fn>>(
                            std::get<Object>(chunk().values[read_byte()]));
                    ...
                    case OpCode::GET_PROPERTY: {
                        if (!std::holds_alternative<Object>(peek(0)) ||
                            !std::holds_alternative<std::shared_ptr<Instance>>(
                                std::get<Object>(peek(0)))) {
                            runtimeError("Only instances have properties.");
                            return interpretResult::RUNTIME_ERROR;
                        }
                        auto instance =
                            std::get<std::shared_ptr<Instance>>(std::get<Object>(peek(0)));
                        auto name = std::string_view{
                            std::get<std::string>(chunk().values[read_byte()]) };
                    ...
                    case OpCode::SET_PROPERTY: {
                        if (!std::holds_alternative<Object>(peek(1)) ||
                            !std::holds_alternative<std::shared_ptr<Instance>>(
                                std::get<Object>(peek(1)))) {
                            runtimeError("Only instances have fields.");
                            return interpretResult::RUNTIME_ERROR;
                        }
                        auto instance =
                            std::get<std::shared_ptr<Instance>>(std::get<Object>(peek(1)));
                        auto name = std::string_view{
                            std::get<std::string>(chunk().values[read_byte()]) };
                    ...
                    case OpCode::INHERIT: {
                        if (!std::holds_alternative<Object>(peek(1)) ||
                            !std::holds_alternative<std::shared_ptr<Class>>(
                                std::get<Object>(peek(1)))) {
                            runtimeError("Superclass must be a class.");
                            return interpretResult::RUNTIME_ERROR;
                        }
                        auto subclass =
                            std::get<std::shared_ptr<Class>>(std::get<Object>(peek(0)));
                        auto superclass =
                            std::get<std::shared_ptr<Class>>(std::get<Object>(peek(1)));
                    ...
                    case OpCode::GET_SUPER: {
                        ...
                        auto superclass =
                            std::get<std::shared_ptr<Class>>(std::get<Object>(pop())).get();
