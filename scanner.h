...
namespace Lox {
	...
	struct Scanner {
			...
			while (true) {
				...
				case '/':
					if (peekNext() == '/') {// A comment goes until the end of the line.
						...
						if (isAtEnd()) errorToken("Unterminated block comment.");
						else advance();
					}
					else { return; }
					break;
		...
		Token identifier() {
			for (auto c = peek(); std::isalpha(c) || std::isdigit(c) || c == '_'; c = peek()) advance();
		...
		TokenType identifierType() {
			switch (start[0]) {
			case 'a': return checkKeyword(1, "nd", TokenType::kw_and);
			case 'b': return checkKeyword(1, "reak", TokenType::kw_break);
			case 'c':
				if (current - start > 1) {
					switch (start[1]) {
					case 'l': return checkKeyword(2, "ass", TokenType::kw_class);
					case 'o': return checkKeyword(2, "ntinue", TokenType::kw_continue);
					}
				}
				break;
			case 'e':
				if (current - start > 1) {
					if (start[1] == 'l')
						switch (start[2]) {
						case 's': return checkKeyword(3, "e", TokenType::kw_else);
						case 'i': return checkKeyword(3, "f", TokenType::elif);
						}
				}
				break;
			case 'f':
				if (current - start > 1) {
					switch (start[1]) {
					case 'a': return checkKeyword(2, "lse", TokenType::kw_false);
					case 'o': return checkKeyword(2, "r", TokenType::kw_for);
					case 'u': return checkKeyword(2, "n", TokenType::fun);
					}
				}
				break;
			case 'i': return checkKeyword(1, "f", TokenType::kw_if);
			case 'n': return checkKeyword(1, "il", TokenType::nil);
			case 'o': return checkKeyword(1, "r", TokenType::kw_or);
			case 'p': return checkKeyword(1, "rint", TokenType::print);
			case 'r': return checkKeyword(1, "eturn", TokenType::kw_return);
			case 's': return checkKeyword(1, "uper", TokenType::super);
			case 't':
				if (current - start > 1) {
					switch (start[1]) {
					case 'h': return checkKeyword(2, "is", TokenType::kw_this);
					case 'r': return checkKeyword(2, "ue", TokenType::kw_true);
					}
				}
				break;
			case 'v': return checkKeyword(1, "ar", TokenType::var);
			case 'w': return checkKeyword(1, "hile", TokenType::kw_while);
			}
			return TokenType::identifier;
		}
